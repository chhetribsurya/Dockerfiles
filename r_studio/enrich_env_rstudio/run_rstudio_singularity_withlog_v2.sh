...
# Purpose: Run RStudio Server via Singularity locally or through SLURM
# ------------------------------------------------------------------------------

# -------------------- USER DEFAULTS (can be overridden) ------------------------

DOCKER_IMAGE="docker://chhetribsurya/enrich_env_rstudio:1.0"
...
Run it via SLURM (with custom options):
  ./run_rstudio_singularity.sh --slurm --partition=short --cpus=4 --mem=16G --time=02:00:00
...
- You can change the password with: RSTUDIO_PASSWORD="yourpass"
- Logs are saved to: ~/rstudio_singularity_YYYYMMDD_HHMMSS.log
- If you're on a remote HPC system, use SSH tunneling:
